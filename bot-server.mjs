...
const PORT = process.env.BOT_PORT ? parseInt(process.env.BOT_PORT) : 3104;
const app = express();
  ...
  switch (cmd) {
    ...
    case '/help': {
      const helpText = [
        ...
        '• <code>/restart</code>: Khởi động lại toàn bộ hệ thống (Port 3100-3104)',
        '',
    ...
    case '/restart': {
      ...
      setTimeout(async () => {
        const p3100 = await checkPort(3100);
        const p3101 = await checkPort(3101);
        const allOk = p3100 && p3101;

        if (allOk) {
          await sendTelegramMessage(
            '✅ <b>HỆ THỐNG ĐÃ KHỞI ĐỘNG LẠI THÀNH CÔNG!</b>\n🟢 Dashboard 3100: Online\n🟢 Bridge ChatGPT & FB Cá Nhân 3101: Online\n🚀 Hệ thống đã sẵn sàng nhận việc!',
            chatId,
          );
        } else {
          await sendTelegramMessage(
...
async function getSystemStatusText() {
  const [p3100, p3101, p5678] = await Promise.all([
    checkPort(3100),
    checkPort(3101),
    checkPort(5678), // n8n port mặc định
  ]);

  const [c9322, c9323, c9324, c9342] = await Promise.all([
    checkPort(9322), // ChatGPT 1
    checkPort(9323), // Facebook Cá Nhân Nick 1
    checkPort(9324), // Facebook Cá Nhân Nick 2
    checkPort(9342), // ChatGPT 2 Quota Fallback
  ]);
  ...
  return [
    ...
    `• Dashboard (3100): ${p3100 ? '🟢 Online' : '🔴 Mất kết nối'}`,
    `• Bridge ChatGPT & FB Cá Nhân (3101): ${p3101 ? '🟢 Online' : '🔴 Mất kết nối'}`,
    `• n8n Workflow (5678): ${p5678 ? '🟢 Online' : '⚪ Chưa bật'}`,
    '',
    '🌐 <b>Chrome Debugging & Nick FB/AI:</b>',
    `• ChatGPT 1 (9322): ${c9322 ? '🟢 Sẵn sàng' : '⚪ Đang tắt'}`,
    `• ChatGPT 2 Fallback (9342): ${c9342 ? '🟢 Sẵn sàng' : '⚪ Đang tắt'}`,
    `• Facebook Cá Nhân Nick 1 (9323): ${c9323 ? '🟢 Sẵn sàng' : '⚪ Đang tắt'}`,
    `• Facebook Cá Nhân Nick 2 (9324): ${c9324 ? '🟢 Sẵn sàng' : '⚪ Đang tắt'}`,
    '',
    '📈 <b>Tiến độ nuôi nick hôm nay:</b>',
...
async function runWatchdogCheck() {
  ...
  if (botConfig.alertOnServerDown) {
    const servers = [
      { port: 3101, name: 'Server 1 (ChatGPT & Facebook Cá Nhân)' },
    ];
...
// -------------------------------------------------------------
// INTERNAL EXPRESS ROUTES (Port 3104)
// -------------------------------------------------------------
  ...
  try {
    ...
    if (hasAnyTrigger && lastAutoPilotRunSlot !== currentSlot) {
      ...
      const triggeredNames = [
        triggeredChannels.personal ? 'Trang Cá Nhân (Nuôi nick)' : null,
