      ...
      accounts: [
        {
          ...
          profileDir: (raw.profileDir as string) || 'n8n-fb-profile-9323',
          port: (raw.port as number) || 9323,
...
export async function GET() {
  try {
    ...
    const chatgptItems = (chatgptConfig.accounts || []).map((acc, index) => {
      ...
      const port = (acc.port as number) || (num === 1 ? 9322 : 9340 + num);
    ...
    (fanpageConfig.accounts || []).forEach((fp, idx) => {
      ...
      const profileDir = fp.profileDir || (p > 0 ? `n8n-fb-profile-${p}` : `n8n-fb-profile-9323`);
      const profilePath = path.join(localAppData, profileDir);
  ...
  try {
    ...
    if (action === 'add_chatgpt') {
      ...
      const newAcc = {
        ...
        port: port ? Number(port) : (nextId === 1 ? 9322 : 9340 + nextId),
