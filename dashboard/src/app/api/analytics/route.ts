...
export async function GET(req: NextRequest) {
  try {
    ...
    const byGpt = {
      acc1: { name: 'ChatGPT Tài khoản 1 (Port 9322)', count: 0, errorCount: 0 },
      acc2: { name: 'ChatGPT Tài khoản 2 (Port 9342)', count: 0, errorCount: 0 },
    ...
    for (const e of dateFilteredEntries) {
      ...
      if (e.chatgptAccount) {
        if (e.chatgptAccount.includes('1') || e.chatgptAccount.includes('9322') || e.chatgptAccount.includes('9222')) {
          byGpt.acc1.count++;
          if (e.status === 'failed') byGpt.acc1.errorCount++;
        } else if (e.chatgptAccount.includes('2') || e.chatgptAccount.includes('9342') || e.chatgptAccount.includes('9242')) {
          byGpt.acc2.count++;
    ...
    if (chatgpt && chatgpt !== 'all') {
      filtered = filtered.filter((e) => {
        ...
        if (chatgpt === 'acc1') return e.chatgptAccount.includes('1') || e.chatgptAccount.includes('9322') || e.chatgptAccount.includes('9222');
        if (chatgpt === 'acc2') return e.chatgptAccount.includes('2') || e.chatgptAccount.includes('9342') || e.chatgptAccount.includes('9242');
        return true;
      });
    }
