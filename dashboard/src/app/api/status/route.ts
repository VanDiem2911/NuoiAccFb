...
export async function GET() {
  try {
    ...
    const gpt1Port = gptPorts[0] || 9322;
    const gpt2Port = gptPorts[1] || 9342;

    const [srv3101, gpt1, gpt2] = await Promise.all([
    ...
    return NextResponse.json({
      ok: true,
      servers: {
        fanpageGpt: { port: ports.fanpageServer, name: 'ChatGPT & FB Cá Nhân', active: srv3101 },
