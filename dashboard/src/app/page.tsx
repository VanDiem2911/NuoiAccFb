...
export default function DashboardPage() {
  const [activeTab, setActiveTab] = useState<'overview' | 'analytics' | 'accounts' | 'credentials' | 'schedule' | 'bot'>('overview');
  ...
  const [newChatGptForm, setNewChatGptForm] = useState<ChatGptModalFormData>({
    name: '',
    profileDir: '',
    port: '',
    enabled: true,
  ...
  const [editingChatGpt, setEditingChatGpt] = useState<EditingChatGptFormData | null>(null);
  ...
  const [newFanpageForm, setNewFanpageForm] = useState({
    name: '',
    pageUrl: '',
    profileDir: 'n8n-fb-profile-9325',
    port: '9325',
    description: '',
    enabled: true,
  });
  ...
  const [fbModalForm, setFbModalForm] = useState<FbModalFormData>({
    ...
    url: '',
    port: 9323,
    profileDir: 'n8n-fb-profile-9323',
    canPostFanpage: true,
    fanpageUrl: '',
    canPostGroup: true,
    groupUrlsText: '',
  ...
  const handleExecuteFullWorkflow = async () => {
    try {
      ...
      if (data.ok && data.result) {
        setTriggerResult(data.result);
        const resList: PublishChannelResult[] = data.result.publishResults || [];
        const groupsRes = resList.find((r) => r.channel === 'groups');
        const fanpageRes = resList.find((r) => r.channel === 'fanpage');
        const personalRes = resList.find((r) => r.channel === 'personal');
  ...
  const handleCreateFanpage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const inputPort = Number(newFanpageForm.port) || 0;
      const finalPort = inputPort > 0 ? inputPort : 9325;
      const finalProfileDir = newFanpageForm.profileDir?.trim() || `n8n-fb-profile-${finalPort}`;
  ...
  const handleUpdateFanpage = async (e: React.FormEvent) => {
    ...
    try {
      const finalPort = Number(editingFanpage.port) || 9323;
      const finalProfileDir = editingFanpage.profileDir?.trim() || `n8n-fb-profile-${finalPort}`;
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // XÁC ĐỊNH PROFILE DIR RIÊNG CHO MỖI TÀI KHOẢN
  ...
  const handleUpdateAccount = async (e: React.FormEvent) => {
    ...
    try {
      const profileDir = editingAccount.profileDir?.trim() || 'n8n-fb-group-profile-1';
