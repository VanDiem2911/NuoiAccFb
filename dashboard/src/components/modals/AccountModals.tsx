...
export interface ChatGptModalFormData {
  id?: string;
  name: string;
  port?: number | string;
  profileDir: string;
...
export interface EditingChatGptFormData {
  id: string;
  name: string;
  profileDir: string;
  port: number;
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg p-6 space-y-5 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveFbAccount} className="space-y-4">
              ...
              <div className="space-y-2.5">
                ...
                {/* Option: Nuôi nick Trang Cá Nhân */}
                <div className="p-3 rounded-2xl border bg-emerald-50/60 border-emerald-300 ring-1 ring-emerald-400/20">
                  <div className="flex items-center gap-2.5">
                    <span className="w-4 h-4 rounded text-emerald-600 font-bold flex items-center justify-center">✓</span>
                    <span className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                      <Users className="w-4 h-4 text-emerald-600" />
                      Dùng để nuôi nick & đăng lên Trang Cá Nhân (Chống Checkpoint)
                    </span>
                  </div>
                </div>
