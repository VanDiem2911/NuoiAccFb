...
export default function ScheduleTab({
  ...
  const [selectedNewPersonalAccounts, setSelectedNewPersonalAccounts] = useState<string[]>([]);
  const [selectedNewPersonalSheet, setSelectedNewPersonalSheet] = useState<string>('topics');

  const personalAccounts = React.useMemo(() => {
    const list: Array<{ id: string; name: string; port?: number }> = [];
    (accounts || []).forEach((cat) => {
      if (cat.category === 'facebook' || cat.category === 'personal' || cat.category === 'fanpage') {
  ...
  const toggleNewPersonalAccount = (accId: string) => {
    setSelectedNewPersonalAccounts((prev) =>
      prev.includes(accId) ? prev.filter((id) => id !== accId) : [...prev, accId]
    );
  };

  const selectAllNewPersonalAccounts = () => {
    setSelectedNewPersonalAccounts(personalAccounts.map((a) => a.id));
  };

  const clearNewPersonalAccounts = () => {
    setSelectedNewPersonalAccounts([]);
  };
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
            ...
            <div className="rounded-3xl p-6 md:p-8 relative overflow-hidden bg-white border border-slate-200 shadow-sm text-slate-900">
              ...
              <div className="mt-6 pt-5 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                  <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                    <Radio className="w-3.5 h-3.5 text-emerald-600" /> Scheduler Daemon (Port 3104)
                  </div>
                  <div className="text-sm font-semibold text-slate-900 mt-2 flex items-center gap-2">
                    {isSchedulerRunning ? (
                      ...
                      <>
                        ...
                        <span className="text-amber-700">Chưa bật Bot Server (Port 3104)</span>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="liquid-glass rounded-3xl p-6 md:p-8 space-y-6 border border-slate-200/80 shadow-lg relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
                ...
                <div className={`p-4 rounded-2xl border transition-all relative ${
                  ...
                  <h4 className="text-xs font-extrabold text-slate-800">4. Đăng Facebook Cá Nhân</h4>
                  <p className="text-[11px] text-slate-500 mt-1 leading-snug">
                    Tự động nuôi nick an toàn, chống checkpoint:
                  </p>
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    <button
                      ...
                      title="Bấm để Bật/Tắt FB Cá Nhân (Nuôi nick)"
                    >
                      {scheduleConfig.channels?.personal ? '✓ FB Cá Nhân (ĐANG BẬT)' : '✕ FB Cá Nhân (ĐÃ TẮT)'}
                    </button>
                  </div>
                  {scheduleTriggering && liveProgress?.step === 4 && (
                    <div className="mt-2.5 flex items-center gap-1.5 text-[10px] font-bold text-indigo-700">
                ...
                <div className="p-5 rounded-2xl bg-emerald-50/90 border-2 border-emerald-400 shadow-md space-y-4 animate-in fade-in duration-300">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2 border-t border-emerald-200">
                    <div className="p-2.5 rounded-xl bg-white/80 border border-emerald-200 text-xs">
                      <span className="text-slate-500">Facebook Cá Nhân:</span>{' '}
                      <span className="font-bold text-emerald-700">
                        {triggerResult.publishResults?.personal?.success ? '✓ Đã đăng Trang cá nhân' : (triggerResult.publishResults?.personal ? '✕ Thất bại' : 'Đang xử lý / Hoàn tất')}
                      </span>
                    </div>
                    <div className="p-2.5 rounded-xl bg-white/80 border border-emerald-200 text-xs">
            ...
            <div className="liquid-glass rounded-3xl p-6 md:p-8 space-y-6 border border-slate-200/80 shadow-md">
              ...
              {workflowActiveTab === 'canvas' && (
                <div className="space-y-6">
                  <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-200/80 text-xs text-emerald-900 font-medium flex items-center gap-2.5">
                    <Clock className="w-5 h-5 text-emerald-600 shrink-0" />
                    <span>
                      <b>Lịch Nuôi Nick Facebook Cá Nhân:</b> Tự động đăng bài theo lịch trình lên từng trang Facebook cá nhân để nuôi nick, duy trì tương tác tự nhiên và chống checkpoint an toàn tuyệt đối với profile độc lập.
                    </span>
                  </div>

                  <div className="w-full">
                    {/* CARD DUY NHẤT: PERSONAL SCHEDULE - NUÔI NICK CHỐNG CHECKPOINT */}
                    <div className="p-6 rounded-2xl bg-emerald-50/60 border border-emerald-200/80 shadow-xs space-y-5">
                      <div className="flex items-center justify-between border-b border-emerald-200/80 pb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <h4 className="font-extrabold text-base text-slate-900">Trang Facebook Cá Nhân (Nuôi nick)</h4>
                            <span className="text-xs font-bold text-emerald-700">Cổng 3101 • Chạy Profile độc lập chống checkpoint tuyệt đối</span>
                          </div>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => handleToggleChannel('personal')}
                          className="flex items-center gap-2.5 cursor-pointer group select-none p-1.5 rounded-xl hover:bg-emerald-100/50 transition-all"
                          title={scheduleConfig.channels?.personal ? 'Bấm để TẮT nuôi nick cá nhân' : 'Bấm để BẬT nuôi nick cá nhân'}
                        >
                          <span className={`text-xs font-extrabold px-2.5 py-0.5 rounded-full transition-all ${
                            scheduleConfig.channels?.personal ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-500'
                          }`}>
                          ...
                          <div className={`w-12 h-6 rounded-full p-0.5 transition-colors duration-200 ease-in-out ${
                            scheduleConfig.channels?.personal ? 'bg-emerald-600' : 'bg-slate-300'
                          }`}>
                            <div className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out ${
                              scheduleConfig.channels?.personal ? 'translate-x-6' : 'translate-x-0'
                            }`} />
                          </div>
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {/* Next Run Info */}
                        <div className="bg-white/90 p-3 rounded-xl border border-emerald-100 text-xs">
                          ...
                          <div className="font-extrabold text-emerald-900 text-sm mt-0.5">
                            {channelNextRuns.personal?.label || 'Chưa xác định (Lịch đang tắt)'}
                          </div>
                        </div>

                        {/* Sheet Source Selection for Personal */}
                        <div className="bg-white/90 p-3 rounded-xl border border-emerald-100 flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            <FileSpreadsheet className="w-4 h-4 text-emerald-600" />
                            <span className="text-slate-700 font-bold">Nguồn bài từ Google Sheet:</span>
                          </div>
                          <select
                            ...
                            className="bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-extrabold rounded-lg px-2.5 py-1 outline-hidden cursor-pointer"
                          >
                            {availableSheets.map((s) => (
                      ...
                      {/* List of Times with Sheet & Multi-Account Selection */}
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <label className="text-xs font-extrabold text-slate-800">
                            Khung giờ nuôi nick &amp; phân bổ tài khoản Facebook cá nhân:
                          </label>
                          <span className="text-[11px] font-bold text-emerald-700 bg-emerald-100/70 px-2 py-0.5 rounded-md">
                            Mỗi giờ có thể phân công 1 hoặc nhiều nick chạy riêng
                          </span>
                        </div>

                        <div className="space-y-2.5">
                          {(scheduleConfig.channelSchedules?.personal?.times || ['08:30', '11:30', '14:30', '19:30']).map((t: string) => {
                            const rawAccounts = scheduleConfig.channelSchedules?.personal?.accountsByTime?.[t]
                              ?? scheduleConfig.channelSchedules?.personal?.accountByTime?.[t];
                            const slotAccounts: string[] = Array.isArray(rawAccounts)
                              ? rawAccounts
                              : (rawAccounts ? [String(rawAccounts)] : []);

                            const currentSheet = scheduleConfig.channelSchedules?.personal?.sheetByTime?.[t]
                              || scheduleConfig.googleSheets?.channelSheetMapping?.personal
                              || scheduleConfig.googleSheets?.sheetName || 'topics';

                            return (
                              <div
                                key={t}
                                className="bg-white p-3.5 rounded-xl border border-emerald-200/90 shadow-2xs space-y-3"
                              >
                                <div className="flex items-center justify-between gap-2 border-b border-emerald-50 pb-2">
                                  <div className="flex items-center gap-2">
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-100 text-emerald-900 font-black text-xs border border-emerald-200 shadow-2xs">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex items-center gap-1">
                                      <span className="text-[11px] text-slate-500 font-semibold">Sheet:</span>
                                      <select
                                        value={currentSheet}
                                        onChange={(e) => handleSlotSheetChange('personal', t, e.target.value)}
                                        className="bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-bold rounded-lg px-2 py-1 max-w-[140px] truncate outline-hidden cursor-pointer"
                                ...
                                <div>
                                  <div className="flex items-center justify-between mb-1.5">
                                    <span className="text-[11px] font-semibold text-slate-500">
                                      Chọn tài khoản FB cá nhân chạy giờ này:
                                    </span>
                                    <div className="flex items-center gap-2">
                                      <button
                                        type="button"
                                        onClick={() => handleSlotAccountsSet?.('personal', t, personalAccounts.map((a) => a.id))}
                                        className="text-[11px] font-bold text-emerald-700 hover:underline cursor-pointer"
                                        title="Chọn tất cả các tài khoản cho giờ này"
                                      >
                                        Tất cả ({personalAccounts.length})
                                      </button>
                                      <span className="text-slate-300">|</span>
                                      <button
                                        type="button"
                                        onClick={() => handleSlotAccountsSet?.('personal', t, [])}
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {personalAccounts.map((acc) => {
                                      ...
                                      return (
                                        <button
                                          key={acc.id}
                                          type="button"
                                          onClick={() => handleSlotAccountsToggle?.('personal', t, acc.id)}
                                    ...
                                    {personalAccounts.length === 0 && (
                                      <span className="text-xs text-slate-400 italic">Chưa có danh sách nick cá nhân trong Cấu hình Tài khoản.</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                      ...
                      {/* Add Time Form */}
                      <div className="bg-white/90 p-4 rounded-xl border border-emerald-200/80 space-y-3">
                        <div className="text-xs font-bold text-slate-700 flex items-center justify-between">
                          <span className="flex items-center gap-1.5">
                            <Plus className="w-4 h-4 text-emerald-600" /> Thêm khung giờ nuôi nick mới (chọn 1 hoặc nhiều nick):
                          </span>
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={selectAllNewPersonalAccounts}
                              className="text-[11px] font-bold text-emerald-700 hover:underline cursor-pointer"
                            >
                              Chọn tất cả
                            </button>
                            ...
                            <button
                              type="button"
                              onClick={clearNewPersonalAccounts}
                              className="text-[11px] font-bold text-slate-500 hover:underline cursor-pointer"
                            >
                              Bỏ chọn
                            </button>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-[11px] font-semibold text-slate-500 mb-1">Giờ đăng</label>
                            <input
                              type="time"
                              value={newPersonalTime}
                              onChange={(e) => setNewPersonalTime(e.target.value)}
                              className="liquid-input rounded-xl px-3 py-1.5 text-xs font-bold text-slate-900 w-full"
                            />
                          </div>
                          <div>
                            <label className="block text-[11px] font-semibold text-slate-500 mb-1">Chọn loại bài (Sheet)</label>
                            <select
                              value={selectedNewPersonalSheet}
                              onChange={(e) => setSelectedNewPersonalSheet(e.target.value)}
                              className="bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-bold rounded-xl px-3 py-1.5 w-full truncate outline-hidden cursor-pointer"
                            >
                              {availableSheets.map((s) => (
                                <option key={s} value={s}>{s}</option>
                              ))}
                        ...
                        <div>
                          <label className="block text-[11px] font-semibold text-slate-600 mb-1.5">
                            Chọn tài khoản FB cá nhân ({selectedNewPersonalAccounts.length > 0 ? `${selectedNewPersonalAccounts.length} nick được chọn` : 'Chưa chọn -> Mặc định nick đầu'}):
                          </label>
                          <div className="flex flex-wrap gap-1.5">
                            {personalAccounts.map((acc) => {
                              const isSelected = selectedNewPersonalAccounts.includes(acc.id);
                              return (
                                <button
                                  key={acc.id}
                                  type="button"
                                  onClick={() => toggleNewPersonalAccount(acc.id)}
                                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold transition-all border cursor-pointer ${
                                    isSelected
                                      ? 'bg-emerald-600 text-white border-emerald-600 shadow-2xs'
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            if (!newPersonalTime) return;
                            handleAddChannelTime('personal', newPersonalTime, selectedNewPersonalAccounts, selectedNewPersonalSheet);
                            setSelectedNewPersonalAccounts([]);
                          }}
                          className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-xs flex items-center justify-center gap-1.5 cursor-pointer transition-colors"
                        >
                          <Plus className="w-4 h-4" /> Thêm Khung Giờ Nuôi Nick Này
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-black shadow-sm flex items-center justify-center gap-1.5 cursor-pointer transition-all disabled:opacity-50"
                      >
                        <Send className="w-3.5 h-3.5" /> 🚀 ĐĂNG NGAY LÊN TRANG CÁ NHÂN (TEST)
                      </button>
                    </div>
