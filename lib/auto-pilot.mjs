...
export function loadScheduleConfig() {
  ...
  return {
    ...
    channels: { personal: true, fanpage: false, groups: false },
    aspectRatio: '4:5',
    hasMascotDu: true,
    topics: [],
    lastRunAt: null,
...
export async function generateImageWithChatGPT(imagePrompt, aspectRatio = '4:5', hasMascotDu = true, preferredAccount = null) {
  try {
    console.log('[AutoPilot] Đang gửi yêu cầu tạo ảnh ChatGPT tới Bridge Port 3101...');
    const bridgePort = process.env.PORT || '3101';
    const res = await fetch(`http://127.0.0.1:${bridgePort}/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (data && data.imageBase64) {
      return {
        ...
        account: data.account || 'ChatGPT Port 3101',
      };
    }
    if (data && data.error) {
    ...
    throw new Error(`Không thể tạo ảnh qua ChatGPT Bridge (3101): ${e.message}`);
  }
}
...
export async function publishToFacebookChannels({ caption, imageBase64, channels = {}, accounts = {} }) {
  ...
  // 1. Facebook Fanpage (Port 3101)
  if (channels.fanpage) {
    try {
      console.log('[AutoPilot] Đang đăng bài lên Facebook Fanpage (Port 3101)...');
      let pageUrl = null;
      let targetProfileDir = null;
      let targetAccountId = null;
      let targetAccountIds = null;

      if (Array.isArray(accounts.fanpage) && accounts.fanpage.length > 0) {
        targetAccountIds = accounts.fanpage;
      } else if (accounts.fanpage) {
        targetAccountId = accounts.fanpage;
      }
      ...
      if (fs.existsSync(FANPAGE_CONFIG_PATH)) {
        try {
          ...
          if (targetAccountId) {
            const selected = (fpConf.accounts || []).find((a) =>
              String(a.id) === String(targetAccountId) ||
              `fb_acc_${a.id}` === String(targetAccountId) ||
              a.name === targetAccountId ||
              (a.port && String(a.port) === String(targetAccountId))
            );
            if (selected) {
      ...
      const res = await fetch(`http://127.0.0.1:${bridgePort}/generate`, {
        ...
        body: JSON.stringify({
          ...
          ...(targetAccountIds ? { accountIds: targetAccountIds } : {}),
  ...
  // 2. Facebook Trang Cá Nhân (Port 3101) - Nuôi nick chống checkpoint
  if (channels.personal || (!channels.fanpage && !channels.groups)) {
    try {
      console.log('[AutoPilot] Đang đăng bài lên Facebook Trang Cá Nhân (Port 3101)...');
      let profileUrl = null;
      let targetProfileDir = null;
      let targetAccountId = null;
      let targetAccountIds = null;

      if (Array.isArray(accounts.personal) && accounts.personal.length > 0) {
        targetAccountIds = accounts.personal;
      } else if (accounts.personal) {
        targetAccountId = accounts.personal;
      }

      if (fs.existsSync(PERSONAL_CONFIG_PATH)) {
        try {
          const persConf = JSON.parse(fs.readFileSync(PERSONAL_CONFIG_PATH, 'utf-8'));
          if (targetAccountId) {
            const selected = (persConf.accounts || []).find((a) =>
              String(a.id) === String(targetAccountId) ||
              `fb_acc_${a.id}` === String(targetAccountId) ||
              a.name === targetAccountId ||
              (a.port && String(a.port) === String(targetAccountId))
            ...
            if (selected) {
              profileUrl = selected.profileUrl || selected.url || selected.pageUrl;
              targetProfileDir = selected.profileDir;
            }
          }
        } catch {}
      ...
      results.push({
        ...
        postUrl: data.postUrl || data.successList?.[0]?.postUrl || data.results?.[0]?.postUrl || null,
        publishedAt: data.publishedAt || data.successList?.[0]?.publishedAt || data.results?.[0]?.publishedAt || null,
        error: data.ok ? null : data.error || 'Lỗi đăng Trang cá nhân',
      });
    } catch (err) {
      results.push({
        channel: 'personal',
  ...
  try {
    if (fs.existsSync(BOT_CONFIG_PATH)) {
      ...
      if (bConf.botToken && bConf.chatId) {
        const botPort = process.env.BOT_PORT || '3104';
        await fetch(`http://127.0.0.1:${botPort}/alert`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
...
export async function runAutoPilotCycle(optionsOrTopic = null) {
  ...
  const effectiveChannels = customChannels || config.channels || { personal: true, fanpage: false, groups: false };
  ...
  console.log(`Kênh hoạt động: Cá nhân: ${Boolean(effectiveChannels.personal)}, Fanpage: ${Boolean(effectiveChannels.fanpage)}`);
  console.log(`=================================================`);
