...
export function logPostActivity({
  ...
  try {
    ...
    if (!computedChannelName) {
      ...
      let sanitizedError = error ? String(error) : null;
      let sanitizedErrorDetails = errorDetails ? String(errorDetails) : null;

      if (sanitizedError) {
        // Strip ANSI escape codes
        sanitizedError = sanitizedError.replace(/\u001b\[[0-9;]*[a-zA-Z]/g, '').replace(/\[\d+m/g, '').trim();
        if (sanitizedError.includes('Call log:')) {
          const parts = sanitizedError.split('Call log:');
          if (!sanitizedErrorDetails) {
            sanitizedErrorDetails = 'Call log:\n' + parts.slice(1).join('Call log:').trim();
          }
          sanitizedError = parts[0].trim();
        }
      }
    ...
    // Bắn cảnh báo tự động về Telegram Bot (Port 3104) nếu gặp lỗi
    if (status === 'failed') {
      try {
        const botPort = process.env.BOT_PORT || '3104';
        fetch(`http://127.0.0.1:${botPort}/alert`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            title: `Thất bại khi đăng trên ${computedChannelName}`,
            channel: computedChannelName,
            targetUrl: targetUrl || targetName,
            details: sanitizedError || sanitizedErrorDetails || 'Tác vụ gặp lỗi',
          }),
        }).catch(() => {});
      } catch {}
    }
