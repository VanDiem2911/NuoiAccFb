...
const chromeDebugUrl = process.env.CHROME_DEBUG_URL || 'http://127.0.0.1:9322';
const app = express();
...
function loadChatGptAccounts() {
  ...
  return [
    { id: 1, name: 'ChatGPT Tài khoản 1', profileDir: 'n8n-chatgpt-profile', port: 9322, enabled: true },
    { id: 2, name: 'ChatGPT Tài khoản 2', profileDir: 'n8n-chatgpt-profile-2', port: 9342, enabled: true },
  ];
}
...
async function publishSingleFacebookPage(account, { caption, imageBase64, mimeType = 'image/png', fileName = 'image.png' }) {
  ...
  try {
    ...
    console.log(`[Fanpage Server 3101] [${account.name}] Hộp thoại đã đóng. Chờ thêm 8 giây để Facebook hoàn tất ghi dữ liệu...`);
    await delay(8000);
    await page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});

    const postUrl = await extractLatestPostUrl(page, targetPageUrl);
    isPublishConfirmed = true;
    console.log(`[Fanpage Server 3101] [${account.name}] ✅ ĐÃ XÁC NHẬN BÀI VIẾT ĐĂNG THÀNH CÔNG 100% LÊN FANPAGE! Link: ${postUrl}`);
    return { ok: true, source: 'facebook-web', account: account.name, pageUrl: targetPageUrl, postUrl, publishedAt: new Date().toISOString() };
  } finally {
    console.log(`[Fanpage Server 3101] [${account.name}] Tác vụ đăng bài hoàn tất. Tự động đóng tab và tắt Chrome cổng ${account.port || 9322}...`);
    try {
      for (const ctx of browser.contexts()) {
        for (const p of ctx.pages()) {
          await p.close().catch(() => {});
    ...
    await closeChromeGracefully(browser, account.port || 9322);
  }
}
...
async function publishFacebookPage(body) {
  ...
  if (!body.accountId && (!body.accountIds || body.accountIds.length === 0) && !body.pageUrl && targetAccounts.length > 1) {
    console.log(`[Fanpage Server 3101] Không chỉ định tài khoản cụ thể. Chỉ đăng tài khoản: "${targetAccounts[0].name}" (Port ${targetAccounts[0].port}) để tránh đăng dồn dập.`);
    targetAccounts = [targetAccounts[0]];
  }

  console.log(`[Fanpage Server 3101] Bắt đầu đăng bài lên ${targetAccounts.length} tài khoản Fanpage...`);
  const successList = [];
  const errorList = [];
  ...
  for (let i = 0; i < targetAccounts.length; i++) {
    ...
    console.log(`\n======================================================`);
    console.log(`[Fanpage Server 3101] [${i + 1}/${targetAccounts.length}] Đang xuất bản lên Fanpage: "${acc.name}" (Port ${acc.port})`);
    console.log(`======================================================`);
    ...
    try {
      ...
      if (i < targetAccounts.length - 1) {
        console.log(`[Fanpage Server 3101] Chờ 5 giây trước khi chuyển sang Fanpage tiếp theo...`);
        await delay(5000);
      }
    } catch (err) {
      console.error(`[Fanpage Server 3101] Lỗi khi đăng Fanpage "${acc.name}":`, err.message);
      errorList.push({
        account: acc.name,
        pageUrl: acc.pageUrl,
        error: err.message,
